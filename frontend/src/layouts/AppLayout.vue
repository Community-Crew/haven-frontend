...
<template>
  <div class="flex flex-col md:flex-row h-screen w-screen bg-haven-white dark:bg-haven-blue select-none overflow-hidden text-haven-black">
    ...
    >
      ...
      <router-link :to="{name: 'user.profile'}" class="p-4 border-t border-white/10 flex items-center shrink-0"
                   :class="isCollapsed ? 'justify-center' : 'space-x-3'">
        <div v-if="user" class="flex items-center gap-3 p-2">
          ...
          <div v-if="!isCollapsed" class="flex flex-col min-w-0 truncate">
    ...
    >
      <div v-if="isMoreMenuOpen" class="md:hidden fixed inset-0 bg-haven-blue text-white z-40 p-6 flex flex-col justify-between pt-20">
        ...
        <router-link :to="{name: 'user.profile'}" @click="closeMoreMenu" class="p-4 bg-white/5 rounded-2xl border border-white/10 flex items-center justify-between mt-4 shrink-0">
